...
SQUID_CI_SERVER=buildmaster.squid-cache.org
...
rsync -rcz --delete-delay rsync://$SQUID_CI_SERVER/snapshots-head/$outfile $SQUID_WWW_PATH/content/Versions/v$ver/ 2>/dev/null
) ||
...
rsync -rcz --delete-delay rsync://$SQUID_CI_SERVER/snapshots-latest/$outfile $SQUID_WWW_PATH/content/Versions/v$ver/ 2>/dev/null ) ||
	echo "rsync pull squid-$ver code snapshots from BuildFarm failed"

ver=$(( $ver - 1 ))
(rsync -rcz --delete-delay rsync://$SQUID_CI_SERVER/snapshots-old/squid-${ver}* $SQUID_WWW_PATH/content/Versions/v$ver/ 2>/dev/null &&
rsync -rcz --delete-delay rsync://$SQUID_CI_SERVER/snapshots-old/$outfile $SQUID_WWW_PATH/content/Versions/v$ver/ 2>/dev/null ) ||
	echo "rsync pull squid-$ver code snapshots from BuildFarm failed"
